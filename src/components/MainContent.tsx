...
export function MainContent({ activeMainNavId = 'home' }: MainContentProps) {
  ...
  const [sortMetric, setSortMetric] = useState('revenue-increase');
          ...
          <div className="flex w-full min-w-0 flex-wrap items-center justify-end gap-2">
            ...
            <div className="flex min-w-0 max-w-full items-stretch">
              <select
                ...
                aria-label="Sort by"
                className="h-12 min-h-12 min-w-[10rem] max-w-[min(100%,18rem)] shrink rounded-l border border-[#e9eaeb] border-r-0 bg-white pl-3 pr-8 font-['Inter',sans-serif] text-sm text-[#101828] outline-none focus:z-10 focus:border-[#0267FF] focus:ring-1 focus:ring-[#0267FF]"
              >
                <option value="revenue-increase">Revenue increase</option>
                <option value="revenue-decrease">Revenue decrease</option>
                <option value="transfer-count">Transfer count</option>
              </select>
              <button
                type="button"
                className="flex h-12 w-12 shrink-0 items-center justify-center rounded-r border border-[#e9eaeb] bg-white text-[#101828] transition-colors hover:bg-slate-50"
                aria-label="Sort order"
              >
                <ArrowDownWideNarrow size={18} strokeWidth={2} aria-hidden />
              </button>
            </div>
            <button
              type="button"
