...
export type RebalancingPrototypeV4Props = {
  prototypeVersion: PrototypeVersionId;
};

export function RebalancingPrototypeV4({
  prototypeVersion,
}: RebalancingPrototypeV4Props) {
  ...
  useEffect(() => {
    ...
    const onKeyDown = (e: KeyboardEvent) => {
      if (e.key === 'Escape') setScopePopoverOpen(false);
    };
    const onPointerDown = (e: MouseEvent) => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="flex w-full min-w-0 flex-col gap-3 rounded-lg border border-[#e3e8f0] bg-[#f8fafc] px-4 py-3 sm:px-5">
          <header className="min-w-0">
            <h1 className="font-['Inter',sans-serif] text-lg font-semibold leading-snug text-[#101828]">
              Rebalancing workspace
            </h1>
            <p className="mt-1.5 max-w-3xl font-['Inter',sans-serif] text-sm leading-relaxed text-[#475467]">
              Review suggested transfers between locations, adjust allocations, and commit when you are ready.
            </p>
          </header>
          {!onboardingChecklistDismissed ? (
            <div className="flex flex-col gap-3 rounded-md border border-[#e3e8f0] bg-white p-3 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
              <ol className="min-w-0 flex-1 list-decimal space-y-1.5 pl-5 font-['Inter',sans-serif] text-sm leading-relaxed text-[#475467] marker:font-semibold marker:text-[#101828]">
                <li>Scan the summary for uplift and how many transfers are suggested.</li>
                <li>Use table tools to sort or filter, then select rows in the grid.</li>
                <li>Generate recommendations or commit from the selection bar at the bottom.</li>
              </ol>
              <button
                type="button"
                onClick={() => setOnboardingChecklistDismissed(true)}
                className="shrink-0 rounded-lg border border-[#e9eaeb] bg-white px-3 py-2 font-['Inter',sans-serif] text-sm font-medium text-[#101828] transition-colors hover:bg-slate-50 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-0 focus-visible:outline-[#0267FF]"
              >
                Got it
              </button>
            </div>
        ...
        <div className="flex w-full min-w-0 flex-col gap-3 pb-4" data-node-id="14764:268954">
          <div className="flex w-full min-w-0 flex-wrap items-center gap-6">
            <div
              ...
              {(
                ...
                return (
                  <button
                    ...
                    aria-description={
                      id === 'products'
                        ? 'Assortment rows and transfer suggestions'
                        : id === 'locations'
                          ? 'Sending and receiving locations'
                          : 'Suggested vehicle trips'
                    }
                    onClick={() => {
          ...
          <div className="h-px w-full shrink-0 bg-[#e9eaeb]" aria-hidden />

          <div className="relative w-full min-w-0" ref={scopeMenuRef}>
            <p className="font-['Inter',sans-serif] text-xs font-semibold uppercase tracking-wide text-[#64748b]">
              Workspace scope
            </p>
            <button
              type="button"
              id={`${scopePopoverId}-trigger`}
              aria-expanded={scopePopoverOpen}
              aria-haspopup="dialog"
              aria-controls={scopePopoverId}
              onClick={() => setScopePopoverOpen((o) => !o)}
              className="mt-1 flex w-full max-w-xl min-w-0 items-center justify-between gap-2 rounded-lg border border-[#e9eaeb] bg-white px-3 py-2.5 text-left font-['Inter',sans-serif] text-sm text-[#101828] transition-colors hover:bg-slate-50 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-0 focus-visible:outline-[#0267FF] sm:w-auto sm:min-w-[16rem]"
            >
              <span className="min-w-0 truncate font-medium">
                {tripType === 'rebalancing' ? 'Rebalancing' : 'Replenishment'}
                <span className="mx-1.5 text-[#D0D5DD]" aria-hidden>
                  ·
                </span>
                {includeZeroTransfers ? 'Zero transfers on' : 'Zero transfers off'}
                <span className="mx-1.5 text-[#D0D5DD]" aria-hidden>
                  ·
                </span>
                Last 7 days
              ...
              <ChevronDown
                size={18}
                strokeWidth={2}
                className={`shrink-0 text-[#667085] transition-transform ${scopePopoverOpen ? 'rotate-180' : ''}`}
                aria-hidden
              />
            </button>
            {scopePopoverOpen ? (
              <div
                id={scopePopoverId}
                role="dialog"
                aria-labelledby={`${scopePopoverId}-trigger`}
                className="absolute left-0 top-full z-40 mt-2 w-[min(100%,22rem)] rounded-lg border border-[#e9eaeb] bg-white p-4 shadow-[0_8px_24px_-4px_rgba(15,23,42,0.12),0_4px_8px_-4px_rgba(15,23,42,0.08)]"
              >
                <p className="font-['Inter',sans-serif] text-xs font-semibold uppercase tracking-wide text-[#64748b]">
                  Trip type
                </p>
                <p className="mt-1 font-['Inter',sans-serif] text-xs leading-relaxed text-[#475467]">
                  Rebalancing optimises stock between locations. Replenishment follows replenishment trip rules
                  (prototype).
                </p>
                <div className="mt-3 inline-flex flex-wrap items-center gap-3" role="group" aria-label="Trip type">
                  <button
                    type="button"
                ...
                <div className="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-[#e9eaeb] pt-4">
                  <span className="text-sm font-normal text-[#4b535c]">Include zero transfers</span>
                  <button
                    type="button"
                    role="switch"
                ...
                <div className="mt-3 flex flex-wrap items-center justify-between gap-3">
                  <span className="text-sm font-normal text-[#4b535c]">Show SOH by unit</span>
                  <button
                    type="button"
                    role="switch"
          ...
          <RebalancingWorkspaceSummaryBanner
            variant="onboarding"
            onOpenKpis={() => setKpisPanelOpen(true)}
          />
          ...
          <div>
            ...
            <button
              ...
              aria-label="Search products in this workspace (prototype)"
              title="Search products (prototype)"
            >
            ...
            <div className="relative shrink-0" ref={filtersMenuRef}>
              <button
                ...
                className="flex h-12 w-12 shrink-0 items-center justify-center rounded-lg border border-[#e9eaeb] bg-white text-[#101828] transition-colors hover:bg-slate-50 focus-visible:z-10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0267FF] focus-visible:ring-offset-0"
                aria-label="Filters"
              >
            ...
            <div className="relative shrink-0" ref={savedFiltersMenuRef}>
              <SavedFiltersToolbarTrigger
                ...
                triggerClassName="h-12 min-h-12 rounded-lg px-4"
                onClick={() => {
                  setFiltersMenuOpen(false);
